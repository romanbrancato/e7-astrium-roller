...
- **Tesseract-OCR** (used to read substat values off the screen, setup below)
...
This launches the GUI (`gui.py`). Click **Refresh Devices** to detect your emulator, then pick a device from the dropdown. If unable to find simply just press Reset ADB connection

There are two tabs:

- **Single Target** Original mode. Rolls for target speed stat
...
## If you have pre-existing substats these instructions must be followed otherwise it will not work 

1. It must be on the beginning astrium screen (just press keep if you are inbetween options)
Example:
![Base gear screen with padlocks, Change Substats, and Confirm Equipment](docs/base-gear-screen.png)

2. You must have the locked substats as Priority #1 and #2 
